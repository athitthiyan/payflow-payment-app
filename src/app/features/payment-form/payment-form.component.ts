              ...
              <div class="processing-steps">
                @for (s of processingSteps; track s.label; let i = $index) {
                  <div class="processing-step" [class.active]="processingStepIdx() >= i" [class.done]="processingStepIdx() > i">
                    <span class="processing-step__icon">{{ processingStepIdx() > i ? '✅' : (processingStepIdx() === i ? '⏳' : '○') }}</span>
                    <span>{{ s.label }}</span>
                  </div>
                }
              </div>
  ...
  styles: [`
    ...
    .demo-btn {
      ...
      &--success {
        ...
        &::before { background: rgba(34,197,94,0.08); }
        &:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(34,197,94,0.2); }
      }
      ...
      &--failure {
        ...
        &::before { background: rgba(239,68,68,0.08); }
        &:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(239,68,68,0.2); }
      }
    }
    ...
    .payment-summary {
      ...
      &__room-loc { font-size: 13px; color: var(--pf-text-muted); }
      ...
      &__amount { display: flex; flex-direction: column; gap: 8px; }
...
export class PaymentFormComponent implements OnInit {
  ...
  formatCardNumber(event: Event) {
    ...
    if (num.startsWith('4')) this.cardBrandIcon.set('💙');
    else if (num.startsWith('5')) this.cardBrandIcon.set('🔴');
    else if (num.startsWith('3')) this.cardBrandIcon.set('💚');
    else this.cardBrandIcon.set('💳');
  }
